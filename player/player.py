...
from dash_media_player.gst_player import GstPlayer
...
class Player(SimpleModule):
    ...
    def __init__(self, id):
        ...
        self.dash_media_player_option = config_parser.get_parameter('dash_media_player')
        self.qoe_function_1_option = config_parser.get_parameter('qoe_function_1_mok')
        self.qoe_function_2_option = config_parser.get_parameter('qoe_function_2_yin')

        # keep track of how many segments were downloaded
        ...
        self.log_writer_thread = threading.Thread(target=self.log_writer.run)
        self.log_writer_thread.start()
        self.all_segments_played = False
        
        if self.dash_media_player_option.lower() == 'on':
            self.dash_media_player = GstPlayer()          
            self.started_dash_media_player = False
            self.dash_media_player_paused = False

            if 'BigBuckBunny' in self.url_mpd:
                init_filename = self.url_mpd.split('/')[6]
                init_filename = init_filename.split('_')
                init_filename = init_filename[0] + '_' + init_filename[1]
            else:
                init_filename = self.url_mpd.split('/')[7]
                init_filename = init_filename.split('_')
                init_filename = init_filename[0] + '_' + init_filename[1]
            print(init_filename)
            self.video_init = self.open_init_segment_file(init_filename)

    def open_init_segment_file(self, init_filename):
        init_file = init_filename + "_init.mp4"
        init_file = open('./init_video_files/' + init_file, "rb")
        init_segment_file = init_file.read()
        init_file.close()
        return init_segment_file

    def push_video_to_dash_media_player(self, msg):
        video_segment = msg.get_payload()

        video_data = self.video_init + video_segment

        # adding the segment in the buffer
        self.dash_media_player.push(video_data)

    def dash_media_player_playback(self):
        # print('QueuedBytes inside decoder: ', self.dash_media_player.get_queued_bytes())
        if self.dash_media_player.get_queued_bytes() > 0 and not self.started_dash_media_player:
            # print("====================> Starting playback from decoder")
            self.dash_media_player.play_segment()
            self.started_dash_media_player = True
        
        elif self.dash_media_player_paused:
            # print("====================> PLAYING after paused")
            self.dash_media_player.play()
            self.dash_media_player_paused = False

        # elif self.dash_media_player.get_queued_bytes() <= 0 and self.started_dash_media_player:
        #     print("====================> PAUSING (consuming faster than producing buffer)")
        #     self.dash_media_player.pause()
        #     self.dash_media_player_paused = True

    def close_dash_media_player_playback(self):
        if self.dash_media_player.get_queued_bytes() == 0 and self.all_segments_played:
            # print("====================> Closing decoder!")
            self.dash_media_player.stop()
            time.sleep(1)
            self.dash_media_player.quit_main_loop()
            self.started_dash_media_player = False
    ...
    def handle_video_playback(self):
        while True:
            ...
            if buffer_size > 0:
                ...
                # print(f'Qualidades selecionadas: {self.playback_quality_qi}')
                ...
                if self.dash_media_player_option.lower() == 'on':
                    self.dash_media_player_playback()
            ...
            if (not threading.main_thread().is_alive() or self.kill_playback_thread) and buffer_size <= 0:
                ...
                self.all_segments_played = True

                if self.dash_media_player_option.lower() == 'on':
                    time.sleep(self.playback_step)
                    self.close_dash_media_player_playback()
    ...
    def finalization(self):
        ...
        initial_loading_delay = self.playback_quality_qi.get_items()
        initial_loading_delay = round(initial_loading_delay[0][0], 2)
        print(f'Initial Loading Delay: {initial_loading_delay} seconds' )
        ...
        total_lenght_video = self.segments_downloaded * segment_duration
        # print(f'Total Lenght of The Video: {total_lenght_video}')
        rebuffering_frequency = round(self.pauses_number / total_lenght_video, 2)
        print(f'Rebuffering Frequency: {rebuffering_frequency} pauses per second')

        average_rebuffering_duration = 0.0
        total_rebuffering_duration = 0.0

        if self.pauses_number > 1:
            pauses = [i[1] for i in self.playback_pauses.get_items()]
            average_rebuffering_duration = round(statistics.mean(pauses), 2)
            # print(f'Average Rebuffering Duration: {average_rebuffering_duration}')
            total_rebuffering_duration = round(sum(pauses), 2)
        
        print(f'Total Rebuffering Duration: {total_rebuffering_duration} seconds')

        if self.qoe_function_1_option.lower() == 'on':
            lti, lfr, ltr = self.performance_level_qoe_f1(initial_loading_delay, 
                                                        rebuffering_frequency,
                                                        average_rebuffering_duration)
            # print(f'Lti: {lti}')
            # print(f'Lfr: {lfr}')
            # print(f'Ltr: {ltr}')

            qoe_function_1 = 4.23 - (0.0672 * lti) - (0.742 * lfr) - (0.106 * ltr)
            print(f'QoE 1 (Mok): {round(qoe_function_1, 2)}')

        if self.qoe_function_2_option.lower() == 'on':

            video_quality = [i[1] for i in self.playback_quality_qi.get_items()]
            if len(video_quality) > 1:
                average_video_quality = round(statistics.mean(video_quality) , 2)
                
            # print(f'Average Video Quality: {average_video_quality} bps')

            quality_variations = []
            for i in range(len(video_quality) - 1):
                quality_variations.append(abs(video_quality[i + 1] - video_quality[i]))

            if len(quality_variations) > 1:
                average_quality_variations = round(statistics.mean(quality_variations), 2)
                
            # print(f'Average Quality Variations: {average_quality_variations} bps')

            video_qual_vari_penalty = 1
            rebuff_time_penalty = round(self.qi[-1] * (9.54 * math.pow(10, -7)), 2)
            init_load_delay_penalty = rebuff_time_penalty
            # print(f'Rebuffering Time Penalty: {rebuff_time_penalty}')


            qoe_function_2 = average_video_quality - \
                            (video_qual_vari_penalty * average_quality_variations) - \
                            (rebuff_time_penalty * total_rebuffering_duration) - \
                            (init_load_delay_penalty * initial_loading_delay)
            
            print(f'QoE 2 (Yin): {round(qoe_function_2, 2)} ')

        self.whiteboard.add_all_segments_played(self.all_segments_played)
    ...
    def handle_segment_size_response(self, msg):
        ...
        if msg.found():
            ...
            if msg.get_payload() is not None:
                self.push_video_to_dash_media_player(msg)
    ...
    def performance_level_qoe_f1(self, init_load_del, rebuff_freq, avg_rebuff_dur):
        if init_load_del >= 0 and init_load_del <= 1:
            lti_level = 1
        elif init_load_del > 1 and init_load_del <= 5:
            lti_level = 2
        else:
            lti_level = 3 

        if rebuff_freq >= 0 and rebuff_freq <= 0.02:
            lfr_level = 1
        elif rebuff_freq > 0.02 and rebuff_freq <= 0.15:
            lfr_level = 2
        else: 
            lfr_level = 3

        if avg_rebuff_dur >= 0 and avg_rebuff_dur <= 5:
            ltr_level = 1
        elif avg_rebuff_dur > 5 and avg_rebuff_dur <= 10:
            ltr_level = 2
        else:
            ltr_level = 3

        return lti_level, lfr_level, ltr_level
